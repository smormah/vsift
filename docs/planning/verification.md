...
| T-04 | Noise, accent, crosstalk, domain terms/numbers, long recording | WER and critical-term accuracy measured; uncertainty and gaps preserved. P07 increment 3b evidence, **partial** (WER, critical-term accuracy and the measured default profile are increment 3c): the opt-in checkpoint's `p07_local_asr_f08_noise_spanish` stage transcribes the noisy, bilingual F08 clip through `transcript retranscribe` and requires "731" and "identificador"; `p07_local_asr_whole_file` requires F05's terms inside its speech span; confidence is kept `provider_uncalibrated` and silent chunks are recorded as gaps. |
