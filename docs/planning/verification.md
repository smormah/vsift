...
| D-05 | Concurrent installs, interrupted activation, rollback while job uses old runtime | One activation transaction; immutable in-use version retained. Current P06 infrastructure provides a root-wide non-blocking OS lock and rejects linked or incorrectly permissioned lock files; publication, crash recovery, rollback and active-version retention remain open. |
