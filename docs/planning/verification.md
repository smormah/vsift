...
| D-04 | Archive traversal, absolute/drive/UNC paths, links, devices, duplicate names, decompression bomb | Entire extraction stays within staging limits; malicious archive rejected. Current P06 increment validates a bounded complete metadata inventory with portable paths, case-insensitive duplicate detection, exact reviewed link headers and rejection of special entries before extraction; no archive parser, selected-file extraction or contained staging exists yet. |
