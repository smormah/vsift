...
Status: proposed release requirements; R0 dependency scope revised 2026-09-13
under ADR 0014. Applies to the CLI, local state, BYO runtime resolution, native
providers, agent handoff and R0 worker deployment. Read
[baseline findings](baseline-review.md) separately:
...
forged manifests/bundles, model output, user-managed executables/models, contributor
...
provider -> captured output; media -> decoder/model; user-managed executable/model
-> VSift process; artifact -> agent; worker result -> supervisor; source/PR ->
release pipeline.
...
| SEC-02 P0 | PATH/current-directory executable hijack or DLL/library search pollution | Canonical explicit path or filtered PATH; private working directory; sanitized env; disclose BYO provenance and require host-approved identities in strict workers | P-02, D-01/D-03 |
...
| SEC-12 P0 | User-managed provider/model is replaced or incompatible after diagnosis | Check selected identity at use, test bounded compatibility, report unverified desktop provenance; strict workers require host-approved read-only identity | D-02/D-05/D-06 |
| SEC-13 deferred | Malicious archive in a future managed installer | No R0 archive extraction or provider download; a future installer needs a new accepted design and traversal/bomb tests | Not applicable in R0 |
| SEC-14 deferred | Download redirect/resume or proxy credential leak in a future managed installer | No R0 provider/model network transport; a future installer needs TLS, redirect, resume and redaction tests | Not applicable in R0 |
| SEC-15 deferred | Future managed runtime removal/rollback breaks an active job | No R0 provider removal/rollback; VSift never deletes BYO files; future lifecycle requires held identity and concurrency tests | Not applicable in R0 |
...
| SEC-23 P0 | VSift release checksum and binary supplied by the same compromised origin | Trust anchor in reviewed source/release metadata; provenance/signature verification where supported; checksum alone not authenticity | R-SEC02 |
...
bring-your-own provenance; P06 owns BYO compatibility and truthful trust reporting. The strict
...
the Rust process. R0 cannot revoke or update user-managed binaries; a strict worker
must reject disallowed identities through host policy, while desktop users receive
actionable guidance and the trust limitation.

## BYO dependency and distribution policy

Under [ADR 0014](../decisions/0014-r0-bring-your-own-dependencies.md), R0 has no
provider/model installer, download or update path. Users prepare FFmpeg, FFprobe and
an optional compatible whisper.cpp plus local model themselves. Explicit absolute
paths and filtered PATH discovery are validated and reported; desktop BYO origin is
not a verified publisher claim. Script-installed tools outside PATH are supported by
explicit selection. Missing capabilities produce typed manual setup guidance; an
instruction inside media never authorizes a setup or system command. No installer
transport or archive-extraction control is considered implemented by this decision.

Keep VSift's own native/npm distribution separate from user-managed binaries/models.
Use native platform packages selected by a small reviewed launcher; do not silently
fetch model weights in npm lifecycle scripts. Users need a Rust toolchain only when
building from source. Qualify package-manager configurations that omit optional
...
Produce an SBOM covering VSift's Rust graph, launcher and shipped artifacts. Record
qualified BYO provider/model identities in test evidence, not as redistributed VSift
components. Do not infer a user-managed FFmpeg build's licence from VSift's own dual
licence. Release artifacts must map to a protected source commit,
