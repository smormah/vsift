# Delivery log, 2026-09-09 to 2026-09-23

Archived, not maintained. This file preserves the two append-only handoff logs
(`memory/TODO.md` and `memory/project_current_status.md`) exactly as they stood at
`9fe6812`, when P06 was parked and delivery was re-planned by
[ADR 0015](../decisions/0015-r0-delivery-replan.md). The live handoff files now
describe only the current state. Headings below are demoted one level; the text is
otherwise unchanged, so its relative links resolve from `memory/`, not from here.

## Archived `memory/TODO.md`
...
deadline limits; prove failure cleanup before activation; compose the guarded
...
fixture identity, expected FFmpeg/FFprobe build prefixes, 64-KiB per-stream and
transcript limits, 256-KiB generated-audio limit, 60-second media and 180-second
inference deadlines, and 16-kHz mono audio.
...
## Archived `memory/project_current_status.md`
